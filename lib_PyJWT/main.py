import jwt

SECRET = "JackySecret"
ALGORITHM = "HS256"

def hs256_encode(payload, key):
    return jwt.encode(payload, key, ALGORITHM)

def hs256_decode(encoded_text, key):
    return jwt.decode(encoded_text, key, ALGORITHM)


if __name__ == "__main__":
    payload = {
        "name": "TOM",
        "age": 123
    }
    encoded_text = hs256_encode(payload, SECRET)
    print(f"HS256 encode: {encoded_text}")
    plain_text = hs256_decode(encoded_text, SECRET)
    print(f"HS256 decode: {plain_text}")

    try:
        plain_text = hs256_decode(encoded_text, "FakeKey")
    except jwt.exceptions.InvalidSignatureError as exc:
        print("[Failed] ", str(exc))

    # * RS256 --- pip install pyjwt[crypto]
    private_key = b"-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----\n"
    public_key = b"-----BEGIN PUBLIC KEY-----\\nFwIDAQAB\n-----END PUBLIC KEY-----\n"

    encoded = jwt.encode({"some": "payload"}, private_key, algorithm="RS256")
    print(f"RS256 encode: {encoded}")

    plain_text = jwt.decode(encoded, public_key, algorithms=["RS256"])
    print(f"RS256 decode: {plain_text}")

